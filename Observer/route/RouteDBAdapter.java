...
public class RouteDBAdapter extends SafeDrivingDBAdapter{
	...
	public int createRoute(RouteBean routeBean) {
		...
		initialValues.put(COLUMN_ROUTE_ID, routeBean.getRouteId());
		...
		Log.d(TAG, "route id : " + routeBean.getRouteId());
	...
	public Cursor fetchDisportedUniqueRouteByLocationId(int locationId, String axis, int routeId){
		String query = "SELECT * FROM routetable where locationId = "+locationId+" and "+ axis +" !=0 and routeId > "+routeId+" order by routeId";
		
		return mySqliteDB.rawQuery(query, null);
